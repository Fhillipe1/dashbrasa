...
def display_kpis(df):
    """Exibe os KPIs com o estilo correto."""
...
def display_hourly_performance_chart(df):
    """Exibe um gráfico de barras com Faturamento por hora."""
    with st.container():
        st.markdown("<h5 class='card-title'>Faturamento por Hora (Madrugada)</h5>", unsafe_allow_html=True)
        if df.empty: return
        ...
        fig = go.Figure(data=go.Bar(
            ...
            y=hourly_summary['Faturamento'],
            marker=dict(color=hourly_summary['Faturamento'], colorscale='YlOrRd'),
        ...
        fig.update_layout(
            xaxis_title='Hora', yaxis_title='Faturamento (R$)',
            template="plotly_dark", margin=dict(t=30, b=30, l=40, r=40),
            plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)',
            height=300
        )
        st.plotly_chart(fig, use_container_width=True)
