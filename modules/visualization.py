...
def criar_donut_e_resumo_canais(df):
    ...
    # Adicionada verificação de segurança para dataframe vazio
    if df.empty:
...
def criar_boxplot_e_analise_outliers(df):
    ...
    with col1:
        chart = alt.Chart(df).mark_boxplot(extent='min-max', outliers=True).encode(
            x=alt.X('Dia da Semana:N', title=None, sort=['1. Segunda', '2. Terça', '3. Quarta', '4. Quinta', '5. Sexta', '6. Sábado', '7. Domingo']),
            y=alt.Y('Total:Q', title='Valor do Pedido (R$)'),
            tooltip=[alt.Tooltip('Dia da Semana:N', title='Dia'), alt.Tooltip('Total:Q', title='Valor do Pedido', format='$,.2f')]
